...
export function specialistPrompt(
  ...
  if (phase === "ANALYZE")
    return `${common}
OUTPUT SCHEMA
Return one JSON object with exactly the documented keys. Omit optional fields when absent; never use null. IDs are 1-100 characters, titles 8-160, and trigger/actualBehavior/expectedBehavior/impact/causality/disproofQuestion each 8-500. verificationRequests has 1-3 objects with tool as the discriminator, never a function call string or nested arguments object. relatedSymbols has at most 8 strings of 1-100 characters. Line numbers are positive integers; optional startLine must be within 10 lines before line.
