...
Subsequent production runs and repeated full-source replays showed that these initial passing checks were not reliable: truncated discovery searches could permanently poison completion, model citations could name the wrong candidate, and the all-mini configuration sometimes rejected valid test failures or accepted unsupported runtime allegations. The recovery update adds deterministic regression coverage for discovery recovery and candidate-specific citations, gives the judge only independent evidence and a compact investigation target, and uses GPT-4.1 for specialists and final judgment, retaining GPT-4.1 mini for routing. Changing only the judge initially passed both controls but repeated runs still exposed incomplete mini-specialist assessments. The budget cap remains unchanged. These observations supersede any inference of reliability from the initial smoke tests above.

The final configuration completed two consecutive full-source pairs with GPT-4.1 specialists and judge: the original PR produced exactly one Must Fix at `oauth.ts:70` for the conflicting test assertion, and the compatible-test control produced no findings. All four reviews had complete coverage. This validates the reported failure and its control, not general production accuracy; the earlier unsuccessful calibration runs are described above. The $1 per-review cap was not raised.
